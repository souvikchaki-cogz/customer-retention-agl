...
async def http_start_single_analysis(req: func.HttpRequest, durable_client: df.DurableOrchestrationClient) -> func.HttpResponse:
    ...
    """
    logging.info("HTTP trigger received request for single analysis.")
    ...
    try:
        ...
        if not customer_id or not note_text:
            logging.warning("Request body missing 'customer_id' or 'text'.")
    ...
    logging.info("Received single analysis request for customer_id: %s.", customer_id)

    # Construct the event payload for the orchestrator
    event = {
    ...
    try:
        ...
        # Create and return the check status response, which includes the statusQueryGetUri
        return durable_client.create_check_status_response(req, instance_id)

    except Exception as e:
        logging.error("Error starting orchestration: %s", str(e))
        return func.HttpResponse(
...
def orchestrator_event_replay(context: df.DurableOrchestrationContext):
    ...
    logging.info("Orchestration started for customer_id: %s", event.get('customer_id'))
    ...
    try:
        ...
        logging.info("Calling activity: activity_call_text_agent")
        text_res = yield context.call_activity('activity_call_text_agent', event)

        # Vulnerability Guardrail Check
        if text_res.get("vulnerability_detected"):
            keywords = text_res.get("vulnerability_keywords", [])
            logging.warning("VULNERABILITY GUARDRAIL TRIGGERED. Terminating orchestration for customer %s. Keywords: %s", event.get('customer_id'), keywords)
        ...
        logging.info("activity_call_text_agent completed. %s", status_detail)
        ...
        logging.info("Calling activity: activity_fetch_structured")
        features = yield context.call_activity('activity_fetch_structured', {
            "customer_id": event["customer_id"],
            "event_ts": event["ts"]
        })
        logging.info("activity_fetch_structured completed.")
        ...
        logging.info("Calling activity: activity_evaluate_rules")
        ...
        logging.info("activity_evaluate_rules completed.")
        ...
        if eval_result.get("should_emit", False):
            ...
            logging.info("Calling activity: activity_write_lead_card")
            ...
            logging.info("activity_write_lead_card completed.")
        else:
            final_score = round(eval_result.get("score", 0) * 100)
        ...
        logging.error("Orchestration failed with error: %s", e)
...
def activity_call_text_agent(payload: dict) -> dict:
    ...
    """
    logging.info("activity_call_text_agent started.")
    customer_id = payload.get("customer_id")
    try:
        logging.info("Calling text agent for customer_id: %s", customer_id)
        ...
        if not ruleset or ruleset_version == "error":
            logging.error("Cannot process text agent: Ruleset is not loaded.")
        ...
        # Add vulnerability info to the result
        ...
        logging.error("An error occurred in activity_call_text_agent for customer_id %s: %s", customer_id, e)
        raise

@app.activity_trigger(input_name="payload")
def activity_evaluate_rules(payload: dict) -> dict:
    logging.info("activity_evaluate_rules started.")
    try:
        t0 = time.time()
        ...
        logging.info("Evaluating rules for customer_id: %s", customer_id)

        # Unpack the tuple returned by load_active_ruleset
        ruleset, ruleset_version = load_active_ruleset()
        score, details = score_event(ruleset, text_result, features)

        # Create a serializable copy of the features to be returned.
        ...
        logging.info("Rule evaluation completed for customer_id: %s. Score: %s, Threshold: %s, Emit: %s", customer_id, score, THRESHOLD, should_emit)
        logging.info("Metric: lead_score=%s", score)
        logging.info("Metric: lead_emitted=%d", 1 if should_emit else 0)
        
        out = {
            "should_emit": should_emit,
            "score": score,
            "rule_hits_json": details["rule_hits_json"],
            "structured_snapshot_json": serializable_features, # Use the cleaned version
            "explanation_text": details["explanation_text"],
            "agent_version": details["agent_version"],
            "ruleset_version": ruleset_version, # Use the unpacked version
            "note_id": event["note_id"],
            "customer_id": customer_id
        }
        
        latency = (time.time() - t0) * 1000
        logging.info("activity_evaluate_rules completed in %.2f ms.", latency)
        return out

    except KeyError as ke:
        logging.error("Missing key in payload for activity_evaluate_rules: %s", ke)
        raise
    except Exception as e:
        logging.error("An error occurred in activity_evaluate_rules: %s", e)
        raise

@app.activity_trigger(input_name="payload")
def activity_fetch_structured(payload: dict) -> dict:
    """
    Fetches structured data for a customer from dbo.Structured and finds the 
    corresponding advertised rate from dbo.product_rates using the customer's product_name.
    """
    logging.info("activity_fetch_structured started.")
    customer_id = payload.get("customer_id")
    if not customer_id:
        logging.error("No customer_id provided in payload. Failing activity.")
        raise ValueError("No customer_id provided in payload.")

    try:
        logging.info("Fetching structured data for customer_id: %s", customer_id)
        sqlc = SqlClient()

        # Step 1: Fetch customer's specific data from the Structured table
        customer_sql = '''
        ...
        if not customer_data:
            logging.warning("No structured data found for customer_id: %s", customer_id)
            return {"customer_id": customer_id, "error": "No structured data found."}

        # Step 2: Find the correct advertised rate based on the customer's product_name
        ...
        if not advertised_rate:
            logging.warning("No matching advertised rate found for product: '%s'.", product_name)

        # Combine customer data with the dynamically found advertised rate
        ...
        logging.info("activity_fetch_structured completed successfully.")
        customer_data["loan_tenure"] = random.randint(0, 9)
        return customer_data

    except Exception as e:
        logging.error("An error occurred in activity_fetch_structured for customer_id %s: %s", customer_id, e)
        raise

@app.activity_trigger(input_name="payload")
def activity_write_lead_card(payload: dict) -> dict:
    logging.info("activity_write_lead_card started.")
    customer_id = payload.get("customer_id")
    try:
        logging.info("Writing lead card for customer_id: %s", customer_id)
        ...
        logging.info("Successfully wrote lead card for customer_id: %s", customer_id)
        return {"ok": True}
    except KeyError as ke:
        logging.error("Missing key in payload for activity_write_lead_card: %s", ke)
        raise
    except Exception as e:
        logging.error("An error occurred in activity_write_lead_card for customer_id %s: %s", customer_id, e)
        raise